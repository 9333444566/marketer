...
class schedules extends MX_Controller {
	...
	public function ajax_enable_activity(){
		...
		if(!empty($activity)){
			$this->db->update(INSTAGRAM_SCHEDULES, array("status" => 3), array("account_id" => $activity->account_id));
			if($activity->status == 3 || $activity->status == 1){
        		...
        		//Stop all schedule
        		$this->db->update(INSTAGRAM_SCHEDULES, array("status" => 1), array("account_id" => $activity->account_id));
